...
安装同一个扩展，运行 `/pisync`，填入**相同的仓库地址**即可。初始化只完成
仓库连接，**不会改动本机配置**；随后会弹出选择框，由你决定首次拉取方式：

- **智能化拉取**：冲突自动以远端为准，本机独有文件保留；
- **以远端覆盖本机**：本机与远端不一致处全部以远端为准（本机独有文件会被删除）；
- **暂不拉取**：什么都不动，之后随时运行 `/pisync` 再选。

无界面会话（`-p`、rpc、json）无法弹选择框，初始化后不应用任何远端配置，
请到带 UI 的会话里完成选择。
