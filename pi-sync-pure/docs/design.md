...
每台设备拥有一个独立分支，分支名完全由用户决定，推荐使用 `device/<设备名>` 的前缀习惯 (插件不做任何改写与强制)，本地仓库的 HEAD 永远停留在设备分支上。main 分支只是远端的汇聚点，本地不长期驻留 main，需要操作 main 时临时 checkout，操作完切回设备分支。
...
merge 冲突即停时，仓库保持操作前状态，向用户报告冲突路径，由用户自行处理或换用带覆盖参数的形式重试。分支参数使用用户输入的原文，不做任何改写 (device/ 只是推荐前缀)；设备间交换配置可直接 `merge-down <对方分支>`，不必经过 main。

## 初始化与分支认领

初始化只负责建立身份：输入设备分支名 (默认为本机唯一 ID，生成方式见分支模型一节) → clone 远端仓库到 `~/.pi/config-repo/` → 若远端已存在其他分支 (不含 main)，列出清单供用户认领 → 创建设备分支 (一律 orphan 空分支，不基于 main) → 写本地状态文件。init 不做任何数据动作：不 capture、不 push、不覆盖本机——要不要把本机配置存档到设备分支 (push)、要不要从远端恢复 (recover)，是用户随后自己执行的显式操作。初始化不做任何针对 main 的操作。

空仓库首次初始化时，scaffold 写入默认 pi-sync.json 并做一次本地初始提交让 orphan 分支诞生 (不推送)；远端已有配置时不写任何东西。非交互会话中，`init <url> <设备名> --new` 可跳过认领流程强制新建设备分支；分支名的唯一性由用户自己保证，不做同名保护。
