...
初始化只负责建立身份：输入设备名 (默认为本机唯一 ID，生成方式见分支模型一节) → clone 远端仓库到 `~/.pi/config-repo/` → 若远端已存在 `device/*` 分支，列出清单供用户认领 → 创建设备分支 → 写本地状态文件。init 不做任何数据动作：不 capture、不 push、不覆盖本机——要不要把本机配置存档到设备分支 (push)、要不要从远端恢复 (recover)，是用户随后自己执行的显式操作。这样迁移场景下落后的本机配置不会被隐含推送顶掉设备分支，recover 始终拿得到远端的好配置。初始化不做任何针对 main 的操作。
